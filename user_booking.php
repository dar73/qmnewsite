...
if (!empty($txtFromD)) {
    $cond .= " and dDateTime>='$txtFromD' ";
    $execute_query = true;
}
if (!empty($txtFromT)) {
    $cond .= " and dDateTime<='$txtFromT' ";
    $execute_query = true;
}
...
$q = "select * from appointments where 1 " . $cond . "and iCustomerID='$sess_user_id' order by iApptID DESC ";
$r = sql_query($q);
                            ...
                            <tbody>
                                ...
                                if (sql_num_rows($r)) {
                                    for ($i = 1; $o = sql_fetch_object($r); $i++) {
                                        ...
                                        $statusStr = Show_serviceStatus($status);
                                        $Bdate = $o->dDateTime;
                                        $AreaID = $o->iAreaID;
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <td><?php echo str_pad($ADDRESS_ARR[$AreaID]['zip'], 5, '0', STR_PAD_LEFT) . ', ' . $ADDRESS_ARR[$AreaID]['city'] . ', ' . $ADDRESS_ARR[$AreaID]['state']; ?></td>
                                            <td>
                                                <div>
                                                    <?php
                                                    if ($status == 'X') { ?>
