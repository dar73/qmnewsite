...
$recaptcha_response = (isset($_POST['g-recaptcha-response'])) ? $_POST['g-recaptcha-response'] : '';
 if (!empty($first_name) && !empty($last_name) && !empty($c_name) && !empty($phone) && !empty($street) && !empty($email) && !empty($passwd1) && !empty($passwd2) && !empty($state_adr) && !empty($county_adr) && !empty($city_adr) && !empty($source) && !empty($rdISBI))
 {
    ...
    if ($responseData->success) {
        ...
        $q = "INSERT INTO service_providers( dDate,First_name, Last_name, company_name, phone, email_address,password,email_verify_key, email_verify, cStatus,street,state,county,city,cSource,cHaveBI) VALUES (NOW(),'$first_name','$last_name','$c_name','$phone','$email','$txtpassword','$vkey','0','I','$street','$state_adr','$county_adr','$city_adr','$source','$rdISBI')";
        $result = sql_query($q);
