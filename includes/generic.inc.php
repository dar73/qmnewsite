...
function FormatDate($date_val, $flag = "A")
{
	...
	if ($date_val != "" && $date_val != '0000-00-00' && $date_val != "0000-00-00 00:00:00") {
		...
		elseif ($flag == "28")	$date_format = "m/d/Y h:ia";
...
{
	global $IMG_TYPE, $DOC_TYPE, $IMG_FILE_TYPE, $DOC_FILE_TYPE,$ALLOWED_EXTE;
	...
	if ($type == 'P') {
		...
		if (in_array($extension, $ALLOWED_EXTE))
			$str = true;
	}
...
{
	$str = '';
	if (empty($class)) $class = 'form-check-input';
	...
	foreach ($value_arr as $key => $txt) {
		...
		$str .= '<label class="form-check-label" for="' . $ctrl_id . '"><input type="radio" onchange="changeHandler(this);" class=" ' . $class . '"  name="' . $ctrl . '" id="' . $ctrl_id . '" value="' . $key . '" ' . $chk_str . ' ' . $fn_str . ' /> ' . $txt . '<span></span></label> ';
		$str .= '</div>';
	}
