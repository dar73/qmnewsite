...
class GoogleCalendarApi
{
    ...
    public function RefreshAccessToken($client_id, $client_secret, $refresh_token)
    {
        ...
        if ($http_code != 200) {
            ...
            return array('data' => array(), 'msg' => $error_msg);
        }
    ...
    public function GetUserCalendarTimezone($access_token)
    {
        ...
        if ($http_code != 200) {
            ...
            // }
            // throw new Exception('Error ' . $http_code . ': ' . $error_msg);
            return array('data' => array(), 'msg' => 'fail');
        }

        return array('data' => $data['value'], 'msg' => 'success');
    }
    ...
    public function GetCalendarEvents($access_token, $calendar_id)
    {
        ...
        $url_parameters['fields'] = 'items(id,summary,start,end)';
        $url_parameters['singleEvents'] = 'true';
        $url_parameters['orderBy'] = 'startTime';

        // Build the URL for the events endpoint
        ...
        if ($http_code != 200) {
            ...
            return array('data' => array(), 'msg' => 'token_expire');
        }

        // Return the list of events
        return array('data' => $data['items'], 'msg' => 'token_expire');
    }
    ...
    public function CreateCalendarEvent($access_token, $calendar_id, $event_data, $all_day, $event_datetime, $event_timezone)
    {
        ...
        if ($all_day == 1) {
            ...
            $dateTime_start = $event_date . 'T' . $start_time . ':00' . $timezone_offset;
            $dateTime_end = $event_date . 'T' . $end_time . ':00' . $timezone_offset;
        ...
        return array('data' => $data['id'], 'msg' => 'success');
    }


    public function CreateCalendarEventWithInvite($access_token, $calendar_id, $event_data, $all_day, $event_datetime, $event_timezone, $attendees = [])
    {
        $apiURL = self::CALENDAR_EVENT . $calendar_id . '/events';
        ...
        if (!empty($attendees)) {
            ...
            foreach ($attendees as $email) {
                $curlPost['attendees'][] = array('email' => $email);
            }
        }
        ...
        if ($http_code != 200) {
            $error_msg = 'Failed to create event with invite';
            return array('data' => array(), 'msg' => 'token_expire');
        }
