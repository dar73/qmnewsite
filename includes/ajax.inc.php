...
if ($response == "UNIQUE_CODE") {
	...
	if (isset($_GET["mode"]) && isset($_GET["status"]) && isset($_GET["id"])) {
		...
		$valid_modes = array('USERS', 'PACKAGES', 'SERVICEPROVIDERS', 'CHATS', 'CUSTOMERS');
  ...
  INNER JOIN apptime T ON T.Id = A.iAppTimeID where A.cService_status='P' and A.cStatus='A' and date_format(dDateTime,'%Y-%m-%d')>'$today' and iBookingID='$id' ", '3');
	...
	exit;
} elseif ($response == 'GET_STATES') {
	$countryid = $_POST['countryid'];
	$STATE_ARR = GetXArrFromYID("select state_id, state_name from states where country_id='$countryid' order by state_name ", '3');
	$result = FillCombo2022('stateid', '', $STATE_ARR, 'state', 'form-control', 'GetCity2(this.value);');
}elseif ($response== 'GET_STATES_MULTIPLE') {
	$countryid = $_POST['countryid'];
	$STATE_ARR = GetXArrFromYID("select state_id, state_name from states where country_id='$countryid' order by state_name ", '3');
	$result = FillCombo2022('state2', '', $STATE_ARR, 'state', 'form-control', 'GetMultipleCounties(this.value)');
} elseif ($response == 'GET_COUNTIES') {
	$stateid = $_POST['stateid'];
	$COUNTIES = GetXArrFromYID("select county_id,county_name from counties where 1 and state_id='$stateid' order by county_name", '3');
	$result = FillComboMultiSelect('countyid', array(), $COUNTIES, 'county', 'form-control mul', 'GetMultipleCities(this.value);');
} elseif ($response == 'GET_CITY') {
	//$countyid = $_POST['countyid'];
	$countryid = $_POST['countryid'];
	$stateid = $_POST['stateid'];
	// echo "select city_id,city_name FROM cities WHERE county_id='$countyid' and state_id='$stateid' and  country_id='$countryid'";
	// exit;
	$CITY_ARR=GetXArrFromYID("select city_id,city_name FROM cities WHERE 1 and state_id='$stateid' and  country_id='$countryid' order by city_name ", '3');
	$result = FillCombo2022('cityid', '', $CITY_ARR, 'city', 'form-control', '');
}elseif($response== 'GET_CITY_MULTIPLE')
{
	$countryid = $_POST['countryid'];
	...
	// echo "select city_id,city_name FROM cities WHERE county_id='$countyid' and state_id='$stateid' and  country_id='$countryid'";
	// exit;
	$CITY_ARR = GetXArrFromYID("select city_id,city_name FROM cities WHERE 1 and county_id in ($counties) and  country_id='$countryid' order by city_name ", '3');
	// Group cities by county for optgroup support
	$COUNTY_NAMES = array();
	if (!empty($counties)) {
		$county_ids = explode(',', $counties);
		if (count($county_ids)) {
			$q = "select county_id, county_name from counties where county_id in ($counties)";
			$r = sql_query($q, '');
			while (list($cid, $cname) = sql_fetch_row($r)) {
				$COUNTY_NAMES[$cid] = $cname;
			}
		}
	}
	...
	foreach ($CITY_ARR as $city_id => $city_name) {
		// Get county_id for this city
		$county_id = GetXFromYID("select county_id from cities where city_id='$city_id'");
		$county_name = isset($COUNTY_NAMES[$county_id]) ? $COUNTY_NAMES[$county_id] : 'Other';
		$grouped[$county_name][] = array('id' => $city_id, 'name' => $city_name);
	}
	foreach ($grouped as $county_name => $cities) {
		$options .= '<optgroup label="' . htmlspecialchars($county_name) . '">';
		foreach ($cities as $city) {
			$options .= '<option value="' . htmlspecialchars($city['id']) . '">' . htmlspecialchars($city['name']) . '</option>';
		}
		$options .= '</optgroup>';
	}
	$result = '<select name="city[]" id="city" class="form-control mul" multiple onchange="GetMultipleZips(this.value);">' . $options . '</select>';
}elseif ($response == 'GET_ZIPS_MULTIPLE') {
	$CITY_ARR = $_POST['city'];
	if(empty($CITY_ARR))
	{
		$CITY_ARR = '0';
	...
	$ZIPS_ARR=GetXArrFromYID("select zip_code,zip_code FROM zip_codes WHERE city_id in ($CITY_ARR) order by zip_code ", '3');
	$result = FillComboMultiSelect('zipid', array(), $ZIPS_ARR, 'Zip', 'form-control mul', '');
} elseif ($response == 'GET_COUNTY') {
	$stateid = $_POST['stateid'];
	$COUNTY_ARR = GetXArrFromYID("select county_id,county_name from counties where state_id='$stateid' order by county_name", '3');
	$result = FillCombo2022('countyid', '', $COUNTY_ARR, 'county', 'form-control', '');
}
