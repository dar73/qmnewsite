...
$USER_LEVEL_COVERAGE_EDITOR = '6';
$USER_PLATINUM_PLAN_USER = '7';
$USER_LEADERSHIP = '8';
##	USER SESSION VARIABLES	###################################################################################
$logged = 0;
$sess_user_type = 'ADM';
$is_sys_admin = $is_super_admin = $is_vendor = $is_customer = $is_ce = $is_gm = $is_accounts = $is_menu_closed = $sess_ref_type = $is_coverage_editor = $is_platinum_plan_user = $is_leadership = false;
$sess_info_str = '';
...
if (isset($_SESSION[PROJ_SESSION_ID]->log_stat)) // if the session variable has been set...
{
	if ($_SESSION[PROJ_SESSION_ID]->log_stat == "A") {
		...
		switch ($sess_user_level) {
			...
			case $USER_LEVEL_COVERAGE_EDITOR: {
					$is_coverage_editor = true;
					break;
				}
			case $USER_PLATINUM_PLAN_USER: {
					$is_platinum_plan_user = true;
					break;
				}
			case $USER_LEADERSHIP: {
					$is_leadership = true;
					break;
				}
		}
...
$ACCESS_ARR = array(14, 16, 18, 19, 23, 25, 29, 48);
if ($is_vendor) {
	$ACCESS_ARR = array(1, 3, 5, 19, 29, 32, 33, 48);
	$VPLAN = GetXFromYID("select cUsertype from service_providers where id='$sess_user_id' ");
	if ($VPLAN == 'P') $ACCESS_ARR = array(1, 3, 5, 19, 29, 32, 33);
} elseif ($is_customer) {
	$ACCESS_ARR = array(1, 3, 5, 11, 12, 14, 17, 18, 16, 23, 25, 26, 27, 32, 33, 48);
} elseif ($is_accounts) {
	$ACCESS_ARR = array(14, 16, 18, 19, 23, 25, 29, 7, 20, 22, 24, 28, 35, 32, 37, 38, 6, 48);
} elseif ($is_coverage_editor) {
	$ACCESS_ARR = array(1, 2, 6, 7, 18, 19, 20, 22, 23, 24, 25, 26, 27, 28, 29, 32, 34, 35, 36, 37, 38, 39, 41, 43, 48);
} elseif ($is_platinum_plan_user) {
	$ACCESS_ARR = array(1, 2, 6, 7, 18, 19, 22, 23, 24, 25, 26, 27, 28, 29, 32, 34, 35, 36, 37, 38, 39, 41, 43, 48);
