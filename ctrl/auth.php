    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if (isset($_POST["txtusername"]) && isset($_POST["txtpassword"])) // && isset($_POST["btnlogin"]))
        {
            ...
            elseif ($username == '')
                ...
                $u_id = $u_level = 0;
                $q1 = "select iUserID, vName, vPassword, vPic, iLevel, cRefType, iRefID from users where vUName='" . $username . "' and cStatus='A'  ";
                $r = sql_query($q1, 'AUTH.61');


                if (sql_num_rows($r)) {
                    list($u_id, $u_name, $u_pass, $u_pic, $u_level, $ref_type, $ref_id) = sql_fetch_row($r);
                ...
                if ($ret == -1 || $ret == -2) {
                    ...
                    $_SESSION[PROJ_SESSION_ID]->user_type = 1;
                    $_SESSION[PROJ_SESSION_ID]->user_reftype = $ref_type;
                    $_SESSION[PROJ_SESSION_ID]->user_refid = $ref_id;
                    ...
                    $URL = SITE_ADDRESS . "ctrl/home.php";
                    $CURL = SITE_ADDRESS . "ctrl/service_providers_disp.php";
                    if ($u_level == '6') {
