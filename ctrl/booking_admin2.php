...
$disp_url = 'booking_admin2.php';
$edit_url = 'booking_admin2.php';
...
$txtFromD = $txtFromT = $txtkeyword = $state = $cond = $COND2 = $params = $params2 = $cond2 = '';
$SP_IDS = array();
...
HAVING COUNT(CASE WHEN a.cService_status = 'O' THEN 1 END) = COUNT(*)");
...
if (isset($_POST['srch_mode']) && $_POST['srch_mode'] == 'SUBMIT') {
    ...
    $params = '&txtFromD=' . $txtFromD . '&txtFromT=' . $txtFromT . '&txtkeyword=' . $txtkeyword . '&state=' . $state;
    ...
    $params2 = '?txtFromD=' . $txtFromD . '&txtFromT=' . $txtFromT . '&txtkeyword=' . $txtkeyword . '&state=' . $state;
}
...
if (!empty($state)) {
    $AREA_IDs = GetIDString2("SELECT id  FROM areas WHERE state='$state'");
    if (empty($AREA_IDs) || $AREA_IDs == '-1') {
        $AREA_IDs='0';
    }
    $cond .= " and iAreaID in (" . $AREA_IDs . ")";
    $execute_query = true;
}
                                ...
                                <div class="page-title-wrapper">
                                    <form class="form-inline p-3" name="frmSearch" id="frmSearch" action="<?php echo $disp_url; ?>" method="post">
                                        ...
                                        <div class="wm-100 mrm-50  form-group m-1">
                                            <?php echo FillCombo2022('state',$state, $STATE_ARR, ' STATE', 'form-control', '') ?>
                                        </div>
