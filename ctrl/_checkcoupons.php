...
if(sql_num_rows($_r1))
{
    $_q2 = "select * from buyed_leads_dat where vCouponCode='$promocode' and iVendorID='$vendorid' and cStatus='A' ";
    $_r2 = sql_query($_q2);
    if(sql_num_rows($_r2)){
        ...
        if(sql_num_rows($_r3))
        {
            ...
            $_q = "insert into transaction values ('$ID','$bookingid','$appointmentid','$vendorid','','','$FINAL_AMT','".NOW."','online','P')";
            $_r = sql_query($_q, "");
            UnlockTable();
            $data = array(
            ...
            sql_query("INSERT INTO buyed_leads_dat(iTransID, iBookingID, iAppID, vCouponCode, fDisc, iVendorID, dtDate, cStatus) VALUES ('$ID','$bookingid','$appointmentid','$promocode','$perc','$vendorid','".NOW."','P')");

            echo '200~~Success, you got $'.$DISC_AMT.' off on this purchase .~~' . $tacValue.'~~'.$BATCH_ID.'~~'.$AMOUNT.'~~'.$CFEE.'~~'.$FINAL_AMT.'~~'. $DISC_AMT;
            exit;
