...
$PAGE_TITLE2 = 'Platinum Service Providers';
$MEMORY_TAG = "SERVICE PROVIDERS";

$PAGE_TITLE .= $PAGE_TITLE2;
$disp_url = 'platinum_service_providers.php';
...
$txtkeyword = $txtphone =$state= $cmbcleaningstatus = $cond = $params = $params2 = $cmbplan = '';
$srch_style = '';

//SELECT `id`, `pid`, `payment_id`, `amount`, `pdate`, `payment_type`, `payment_status` FROM `transaction2` WHERE 1

$AUTH_GUIDS_ARR=GetXArrFromYID("select pid from transaction2 where payment_status='A' ");
...
if (isset($_POST['srch_mode']) && $_POST['srch_mode'] == 'SUBMIT') {
    ...
    $params = '&keyword=' . $txtkeyword . '&txtphone=' . $txtphone . '&cmbcleaningstatus=' . $cmbcleaningstatus . '&cmbplan=' . $cmbplan. '&state=' . $state;
    ...
    $params2 = '?keyword=' . $txtkeyword . '&txtphone=' . $txtphone . '&cmbcleaningstatus=' . $cmbcleaningstatus . '&cmbplan=' . $cmbplan.'&state=' . $state;
...
if(!empty($state))
{
    $ZIPS = GetIDString2("select zip from areas where state='$state'  ");
    if(empty($ZIPS))
    {
        $ZIPS = '0';
    ...
    $execute_query2 = true;
    $SP_ZIPS_IDS=GetIDString2("select service_providers_id from service_providers_areas where zip in ($ZIPS) ");
    if(!empty($SP_ZIPS_IDS))
    {
        $cond .= " and id in ($SP_ZIPS_IDS) ";
    }else{
        $cond .= " and id='0' ";
    }
}
                                            ...
                                            <div class="page-title-wrapper">
                                                <form class="form-inline p-3" name="frmSearch" id="frmSearch" action="<?php echo $disp_url; ?>" method="post">
                                                    ...
                                                    <div class="wm-100 mrm-50  form-group m-1">
                                                        <?php echo FillCombo2022('state', $state, $STATE_ARR, 'COVERAGE STATE', 'form-control', 'GET_COUNTY(this.value);') ?>
                                                    </div>
                                    ...
                                    <table class="table table-hover " id="cTable" style="width: 100%;">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Calendar access Done</th>
                                                <th>Access Token</th>
                                                <th>Refresh Token </th>
                                                <th>GUID capture Done</th>
                                                <th>Phone</th>
                                                <th>Email Address</th>
                                                <th>Lead/Week</th>
                                                <th>Lead/Month</th>
                                        ...
                                        <tbody>
                                            ...
                                            if (!empty($dataArr)) {
                                                for ($u = 0; $u < sizeof($dataArr); $u++) {
                                                    ...
                                                    $LEAD_WEEK =(isset($LEADS_PER_WEEK[($dataArr[$u]->vLeadPerWeek)]))?$LEADS_PER_WEEK[($dataArr[$u]->vLeadPerWeek)]:"NA";
                                                    $LEAD_PER_MONTH =(isset($LEADS_PER_WEEK[($dataArr[$u]->vLeadPerMonth)]))? $LEADS_PER_WEEK[($dataArr[$u]->vLeadPerMonth)]:"NA";
                                                    $Platinum_agreement = isset($dataArr[$u]->cPlatinumAgreement)?db_output2($dataArr[$u]->cPlatinumAgreement):'';
                                                    $access_token = isset($dataArr[$u]->vAccessToken)?db_output2($dataArr[$u]->vAccessToken):'';
                                                    $refresh_token = isset($dataArr[$u]->vRefreshToken)?db_output2($dataArr[$u]->vRefreshToken):'';
                                                    ...
                                                    if ($Platinum_agreement=='Y') {
                                                        $AGRE='<span class="text-success">Yes</span>';
                                                    }

                                                    if (!empty($access_token) && !empty($refresh_token)) {
                                                        $CALENDAR_A='<span class="text-success">Yes</span>';
                                                    }

                                                    if (in_array($x_id,array_keys($AUTH_GUIDS_ARR))) {
                                                        $AUTH='<span class="text-success">Yes</span>';
                                                    }
                                            ?>
                                                    <tr>
                                                        <td><?php echo $i . '.' ?></td>
                                                        ...
                                                        <td><?php echo $AGRE; ?></td>
                                                        <td><?php echo $CALENDAR_A; ?></td>
                                                        <td><?php echo $access_token; ?></td>
                                                        <td><?php echo $refresh_token; ?></td>
                                                        <td><?php echo $AUTH; ?></td>
                                                        <td><?php echo $x_phone; ?></td>
                                                        <td><?php echo $x_email; ?></td>
                                                        <td><?php echo $LEAD_WEEK; ?></td>
                                                        <td><?php echo $LEAD_PER_MONTH; ?></td>
                                                        ...
                                                        <td style="text-align:center;"><a href="<?php echo $edit_coverage_url; ?>" class="btn btn-danger" target="_blank" rel="noopener noreferrer">Coverage area</a></td>
                                                        <td><button class="btn btn-danger" onclick="DeleteUser('<?php echo $x_id; ?>');"><i class="fa fa-trash"></i></button></td>
