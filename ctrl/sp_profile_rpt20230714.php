...
$dataArr = GetDataFromCOND("service_providers", $cond . " and cStatus!='X' order by id");
$EXCEL_DATA=array(array('Sr.No', 'First Name', 'Last Name', 'Company Name','State','County', 'Phone', 'Email Address', 'Profile Status', 'Email Verify'));
...
if (!empty($dataArr)) {
    for ($u = 0; $u < sizeof($dataArr); $u++) {
       ...
       // $url = $edit_url . '?mode=E&id=' . $x_id;
        ...
        array_push($EXCEL_DATA,array($i, $x_fname, $x_lname, $x_Company_Name,$x_state,$x_county,$x_phone,$x_email,$PERCENTAGE,$EMAIL_VERIFY[$x_emailVerify]));
    }
}
