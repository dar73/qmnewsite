...
$disp_url = 'chat_disp.php';
$edit_url = 'chat_edit.php';
...
if ($mode == 'A') {
    ...
    // DFA($_POST);
    // exit;
    $txtquestion = db_input2($_POST['txtquestion']);
    $txtreply = db_input2($_POST['txtreply']);
    $rdstatus =  db_input2($_POST['rdstatus']);

    LockTable('chatbot_hints');
    $txtid = NextID('id', 'chatbot_hints');
    $q = "INSERT INTO chatbot_hints(id, question, reply,cStatus) VALUES ('$txtid','$txtquestion','$txtreply','$rdstatus')";
    $r = sql_query($q, "USERS.123");
    UnLockTable();

    // $desc_str = 'Newly Created: '.db_input($q);
    // LogMasterEdit2($txtid, 'USR', $mode, $txtname, $desc_str, $sess_user_id);
    $_SESSION[PROJ_SESSION_ID]->success_info = "chat Details Successfully Inserted";
...
// ) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=latin1;
    ...
    $dataArr = GetDataFromID("chatbot_hints", "id", $txtid);
    if (empty($dataArr)) {
        header("location: $disp_url");
        exit;
    }

    $txtquestion = db_output2($dataArr[0]->question);
    $txtreply = db_input2($dataArr[0]->reply);
    ...
    // [mode] => U
    // [add_mode] => N
    // [user_token] => MTY1NDYyNDU2NDg3MzBkZTNkYmE1OS43ODY0MTY2OA==
    // [txtquestion] => what is your name||whats your name
    // [txtreply] => My name is Vishal Bot
    // [rdstatus] => A
    ...
    $values = " question='$txtquestion', reply='$txtreply',cStatus='$rdstatus' ";
    $QUERY = UpdataData('chatbot_hints', $values, "id=$txtid");

    $_SESSION[PROJ_SESSION_ID]->success_info = "Chat Details Successfully Updated";
                                    ...
                                    <form class="" id="usersForm" name="usersForm" method="post" action="<?php echo $edit_url; ?>" enctype="multipart/form-data">
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <div class="form-row">
                                                <div class="col-md-6">
                                                    <div class="position-relative form-group">
                                                        <label for="txtquestion" class="">Question <span class="text-danger">*</span></label>
                                                        <input name="txtquestion" id="txtquestion" type="text" value="<?php echo $txtquestion; ?>" class="form-control form-control-sm radius-30">
                                                    </div>
                                                </div>
                                            ...
                                            <div class="form-row">
                                                ...
                                                <div class="col-md-12">
                                                    <div class="position-relative form-group">
                                                        <label for="reply" class="">Reply</label>
                                                        <textarea class="form-control" rows="3" id="txtreply" name="txtreply"><?php echo $txtreply;?></textarea>
