...
$q = "select * from appointments where 1 and cService_status='P' and iAreaID in (SELECT DISTINCT t1.id FROM areas t1 INNER JOIN service_providers_areas t2 ON t1.zip=t2.zip WHERE 1 AND  t2.service_providers_id='$sess_user_id') " . $cond . "and cService_status='P' and DATE_FORMAT(dDateTime,'%Y-%m-%d') >=NOW() order by dDateTime DESC ";
$r = sql_query($q);
                            ...
                            <tbody>
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <td>$125</td>
                                            <td><button type="button" onclick="req('<?php echo $Bid; ?>','<?php echo $sess_user_id; ?>','125','<?php echo $appID;?>');" data-amt="125" data-Bid="<?php echo $Bid; ?>" data-vID="<?php echo $sess_user_id; ?>" class="stripe-button btn btn-sm btn-primary" id="payButton">
                                                    <i class="fa fa-shopping-cart"></i> Buy Lead
                                                </button> <button type="button" onclick="ShowInfo('<?php echo $Bid; ?>','<?php echo $appID;?>');" class="btn btn-sm btn-info">View Info</button></td>
                                        </tr>
