...
if (empty($BID)) {
    $BID = (isset($_GET['b'])) ? DecodeParam($_GET['b']) : '';
}

if (empty($PID)) {
    $PID = (isset($_GET['p'])) ? DecodeParam($_GET['p']) : '';
}

if (empty($BID) && empty($PID) && empty($APP_ID)) {
    echo 'Invalid Access!!!';
    exit;
}
...
if ($Leads_Ans[0]->iAnswerID == '101' || $Leads_Ans[0]->iAnswerID == '102') {
    ...
    if (NOW < date('Y-m-d H:i:s', strtotime('2024-12-03 00:00:00'))) {

        $DISCOUNT = 20;
    }
    $AMOUNT = 125 - 125 * $DISCOUNT / 100;
} else {
    if (NOW < date('Y-m-d H:i:s', strtotime('2024-12-03 00:00:00'))) {
...
<body>
    ...
    <!-- <strong>Alert!</strong> Sorry our system is experiencing problems, and we are working on a fix. Submit your payment as usual, we will contact you to complete the payment. -->
    <!-- </div> -->
    ...
    <div class="row">
        <div class="col-75 ">
            ...
            <div class="container ">
                ...
                <input type="hidden" name="appointmentid" id="appointmentid" value="<?php echo $APP_ID; ?>">
                <input type="hidden" name="bookingid" id="bookingid" value="<?php echo $BID; ?>">
                ...
                <form action="https://services.epx.com/browserpost/" method="post" name="checkout_form" id="checkout_form">
                    ...
                    <div class="row">
                        ...
                        <div class="col-50">
                            ...
                            <br>
                            <br>
                            <div class="promo-code" style="padding: 50px;border: 2px solid;">
                                <span class="text-danger">If you have promo code then please enter here</span>
                                <input type="text" name="promocode" id="promocode" placeholder="Enter your promo code here">
                                <span id="promocode_span" class="ermsg" style="color:red;font-weight: bold;display: none;">please enter promo code</span>
                                <button class="apply btn btn-primary" onclick="ValidateCouponcode();" type="button">Apply</button>
                            </div>
                        </div>

                    </div>
                    <label>
                        <input type="checkbox" id="check_agree" name="check_agree"> I understand that I will not be able to ask for a credit if I miss the scheduled appointment I am now purchasing.
                    </label>
        ...
        <div class="col-25">
            <div class="container">
                ...
                <?php if (!empty($DISCOUNT)) { ?>
                    <p><a href="#">Discount (<?php echo $DISCOUNT ?>%)</a> <span id="" class="price">$ <?php echo 125 * $DISCOUNT / 100; ?></span></p>
                <?php } ?>
                <p><a href="#">Lead price</a> <span id="LPRICE" class="price">$<?php echo $AMOUNT; ?></span></p>
                <p><a href="#">Convenience fee (3%)</a> <span id="LCFEE" class="price">$<?php echo $CFEE; ?></span></p>
                <!-- <p><a href="#">Product 3</a> <span class="price">$8</span></p>
                <p><a href="#">Product 4</a> <span class="price">$2</span></p> -->
                <hr>
                <p>Total <span id="LTOTAL" class="price" style="color:black"><b>$<?php echo $FINAL_AMT; ?></b></span></p>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function ValidateCouponcode() {
            ...
            if (ret_val) {
                PageBlock();
                var data = 'bookingid=' + bookingid.val() + '&appointmentid=' + appointmentid.val() + '&vendorid=' + vendorid.val() + '&promocode=' + promocode.val() + '&leadprice=' + leadprice.val();
                setTimeout(function() {
                    $.ajax({
                        url: '_checkcoupons.php',
                        async: false,
                        method: 'POST',
                        data: data,
