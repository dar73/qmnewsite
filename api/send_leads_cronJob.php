...
$_q = "select iApptID,iAreaID,dDateTime,iAppTimeID from appointments where cService_status='P' and DATE_FORMAT(dDateTime,'%Y-%m-%d') >=NOW() order by dDateTime DESC ";
...
while (list($iApptID, $iAreaID, $date, $TIMEID) = sql_fetch_row($_r)) {
    ...
    while (list($providersID) = sql_fetch_row($r2)) {
        $q3 = "SELECT email_address FROM service_providers WHERE id='$providersID' and cAdmin_approval='A' and cStatus='A' ";
        $r3 = sql_query($q3);
        ...
        // Additional headers to prevent emails from being flagged as spam
        $headers .= "Sender: ops@janitorialquotemasters.com\r\n";
        $headers .= "Return-Path: ops@janitorialquotemasters.com\r\n";
        $headers .= "X-Sender: ops@janitorialquotemasters.com\r\n";
        ...
        $mail_content = "<html>";
        $mail_content .= "<body>";
        $mail_content .= "<head><title>Leads Alert</title></head>";
        $mail_content .= '<div style=" background-image: url(https://thequotemasters.com/Images/faded-logo-large.png);background-position: center;background-repeat: no-repeat;background-size: contain;background-attachment: fixed;"><p>Hello Quote Master Provider!!</p>';
        ...
        $mail_content .= '<br><p>If '."you'd ".'like to purchase this appointment for {$85} press here  <a href="https://thequotemasters.com/">press here</a></p>';
