...
//DFA($UNSUBSCRIBED_LEADS_ARR);
// echo (in_array('66',$UNSUBSCRIBED_LEADS_ARR['112']));
// exit;
$Lead_price = 0;
...
where  A.cService_status='P' and DATE_FORMAT(A.dDateTime,'%Y-%m-%d') >=NOW()  and A.cStatus!='X'  and B.cStatus='A' and B.bverified='1' ";
...
while (list($BID, $iAreaID, $date, $txtnotes) = sql_fetch_row($_r)) {
    ...
    // echo $iApptID;
    // exit;
    $AREAD_DETAILS = array();//GetDataFromID('areas', 'id', $iAreaID);
    ...
    while (list($providersID) = sql_fetch_row($r2)) {
        ...
        if (!sql_num_rows($spcheck_q_r)) {
            ...
            $q3 = "SELECT email_address,First_name FROM service_providers WHERE id='$providersID' and cAdmin_approval='A' and cStatus='A' and cUsertype='S' ";
            $r3 = sql_query($q3);
            if (!sql_num_rows($r3)) {
                continue;
            }
    ...
    JOIN 
        countries c ON ci.country_id = c.country_id where 1 and  z.zip_code='$zip' ";
            $_qr = sql_query($_qa);
            list($zipCode, $country_name, $state_name, $city_name) = sql_fetch_row($_qr);
            $LEAD_ADD = " " . $city_name . " , " . $state_name . " , " . $zipCode;
            ...
            if (sql_num_rows($_r4)) {
                ...
                if (!is_null($to)) {
                    ...
                    // if(!in_array($BID, $UNSUBSCRIBED_LEADS_ARR[$providersID]) && isset($UNSUBSCRIBED_LEADS_ARR[$providersID]))
                    // {
                    //     echo $MAIL_BODY;
                    //     echo 'Success';
                    // }
