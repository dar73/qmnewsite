...
if (isset($_POST['county_name'], $_POST['state'])) {
    ...
    $q = "SELECT DISTINCT city FROM areas WHERE state='$state' AND County_name IN ('" . implode("','", $county_name) . "') order by city ";
    ...
    $str .= '<select name="city[]" class="form-control " multiple="multiple" data-placeholder="Select a City" id="city">';
    foreach ($county_name as  $value) {
        $Q = "SELECT DISTINCT city FROM areas WHERE state='$state' AND County_name='$value' order by city ";
        $r = sql_query($Q);
