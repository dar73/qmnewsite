...
if (sql_num_rows($cust_u_exist_q_res)) {
    ...
    $cust_q = "INSERT INTO customers  VALUES ('$customerID','$first_name','$last_name','$name_of_company','$c_address','$position','$email','hLn15@)T~r7m}U{o-^q','$phone',NOW(),'N','A')";
    sql_query($cust_q, "");
...
$qr2 = "INSERT INTO booking VALUES ('$bid','$areaID','$customerID','$num_of_Q','Y','P','0','001',NOW(),'$Notes','A')";
sql_query($qr2, "");
$query = array();
...
for ($i = 0, $j = 1; $i < $num_of_Q; $i++, $j++) {
    ...
    $_q2= "INSERT INTO appointments VALUES ('$ApptID','$bid','$areaID','$customerID',NOW(),'0','1','". $dateArr[2] . '-' . $dateArr[0] . '-' . $dateArr[1]. "','" . $_POST['Time' . $j] . "','P','A')";
    sql_query($_q2,'');
}
UnlockTable();
...
SendInBlueMail($subject,$to,$mail_content, '', '','', 'vik@teamleadgeneration.onmicrosoft.com,gemma@teamleadgeneration.onmicrosoft.com');
