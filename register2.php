...
$state_adr = (isset($_POST['stateid'])) ? db_input2($_POST['stateid']) : '';
$county_adr = (isset($_POST['countryid'])) ? db_input2($_POST['countryid']) : '';
$city_adr = (isset($_POST['cityid'])) ? db_input2($_POST['cityid']) : '';
...
if (!empty($first_name) && !empty($last_name) && !empty($c_name) && !empty($phone) && !empty($street) && !empty($email) && !empty($passwd1) && !empty($passwd2) && !empty($state_adr) && !empty($county_adr) && !empty($city_adr) && !empty($source) && !empty($rdISBI)) {
    ...
    if ($responseData->success) {
        ...
        $q = "INSERT INTO service_providers(id,dDate,First_name, Last_name, company_name, phone, email_address,password,email_verify_key, email_verify, cStatus,street,state,county,city,cSource,cHaveBI,cCleaningStatus,cUsertype,vSnotes,vLeadPerMonth,cCalendarAct) VALUES ('$txtspid','$NOW','$first_name','$last_name','$c_name','$phone','$email','$txtpassword','$vkey','0','I','$street','$state_adr','$county_adr','$city_adr','$source','$rdISBI','$cleaningstatus','$plan','$txtbox1','$txt_max_leadsPerMonth','$rdactype')";
        $result = sql_query($q);
        UnLockTable();
            ...
            if (isset($_SESSION[COVERAGE])) {
                foreach ($_SESSION[COVERAGE] as $countryKey => $states) {
                    foreach ($states as $stateKey => $value) {
                        $icoverageID = NextID('iCoverageId', 'coverages');

                        $str1 = '';
                        $state = " state='$stateKey' ";

                        $countystr= '';
                        if(!empty($value['county'])) $countystr =  implode(",", $value['county']);
                        ...
                        if (!empty($value['city'])) {
                            ...
                            sql_query("INSERT INTO coverages(iCoverageId, iproviderID,iCountryID,vStates,vCounties,vCities) VALUES ('$icoverageID','$txtspid','$countryKey','$stateKey','$countystr','" . implode(",", $value['city']) . "')");
                        } else {
                            
                            sql_query("INSERT INTO coverages(iCoverageId, iproviderID,iCountryID,vStates,vCounties,vCities) VALUES ('$icoverageID','$txtspid','$countryKey','$stateKey','$countystr','')");
                        }

                        UpdateCoverages4($txtspid);
